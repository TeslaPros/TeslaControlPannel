...
    @{ Name = 'Mod Analyser';          Icon = [char]0xE943; Kind = 'Command'; Admin = $true;  Cmd = 'powershell -ExecutionPolicy Bypass -Command "Invoke-Expression (Invoke-RestMethod ''https://raw.githubusercontent.com/xkzuto96/xkzutos-mod-analyzer/main/XkzutosModAnalyzer.ps1'')"'; Desc = 'Analyzes Minecraft mods using Mod Analyser.'; ButtonType = 'Neutral' },
...
# Maak de animatie af om het element te verbergen
    $animation = New-Object System.Windows.Media.Animation.DoubleAnimation
    $animation.From = 1
    $animation.To = 0
    $animation.Duration = [System.Windows.Duration]::new([TimeSpan]::FromMilliseconds($DurationMs))
...
foreach ($Tool in $Tools) {
    ...
    $btn.Tag = $Tool.Icon # Dit vult de icoon-placeholder in de XAML
    
    # Kleur aanpassen als het een 'Primary' knop is
    if ($Tool.ButtonType -eq 'Primary') {
        $btn.Background = $window.Resources["PrimaryButtonBrush"]
    }

    # Wat gebeurt er als je op een tool klikt?
    $btn.Add_Click({
        param($sender, $e)
        $currentTool = $Tools | Where-Object { $_.Name -eq $sender.Content }
        ...
        $MainProgressBar.Value = 50 # Simpele indicatie van activiteit

        if ($currentTool.Kind -eq 'Command') {
            # Start het PowerShell commando in een nieuw venster
            Start-Process powershell -ArgumentList "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", $currentTool.Cmd
        } elseif ($currentTool.Kind -eq 'Folder') {
            # Open de map in verkenner
            explorer.exe $currentTool.Path
        }

        # UI herstellen na lancering
    ...
    # Voeg de knop toe aan de lijst in de GUI
