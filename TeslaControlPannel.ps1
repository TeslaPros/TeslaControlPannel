...
$Tools = @(
    @{ Name = 'TeslaPro Macro Finder'; Icon = [char]0xE721; Kind = 'Command'; Admin = $true;  Cmd = 'iex (irm ''https://raw.githubusercontent.com/TeslaPros/TeslaProMacroFinder/main/TeslaProMacroFinder_V3.ps1'')'; Desc = 'Scans the system for macro-related traces and suspicious activity.'; ButtonType = 'Primary' },

    @{ Name = 'Doomsday Detector';     Icon = [char]0xE7BA; Kind = 'Command'; Admin = $true;  Cmd = 'iex (irm ''https://raw.githubusercontent.com/TeslaPros/DoomsdayDetector/main/DoomsdayClientDetectorV3.ps1'')'; Desc = 'Launches the Doomsday client detection workflow.'; ButtonType = 'Neutral' },

    @{ Name = 'Mod Analyser';          Icon = [char]0xE943; Kind = 'Command'; Admin = $true;  Cmd = 'Invoke-Expression (Invoke-RestMethod ''https://raw.githubusercontent.com/xkzuto96/xkzutos-mod-analyzer/main/XkzutosModAnalyzer.ps1'')'; Desc = 'Analyzes Minecraft mods using Mod Analyser.'; ButtonType = 'Neutral' },

    @{ Name = 'VPN Finder';            Icon = [char]0xE836; Kind = 'Command'; Admin = $true;  Cmd = 'iex (irm ''https://raw.githubusercontent.com/TeslaPros/VPNChecker/main/VPNFinder.ps1'')'; Desc = 'Searches for active VPN connections and related traces.'; ButtonType = 'Neutral' },

    @{ Name = 'Security Manager';      Icon = [char]0xE756; Kind = 'Command'; Admin = $true;  Cmd = 'Set-ExecutionPolicy Bypass -Scope Process -Force; iex (irm ''https://pastebin.com/raw/ChxAuDpF'')'; Desc = 'Prepares and launches additional security-oriented tooling.'; ButtonType = 'Neutral' },

    @{ Name = 'QuickCheck Scanner';    Icon = [char]0xEC92; Kind = 'Command'; Admin = $true;  Cmd = 'Set-ExecutionPolicy Bypass -Scope Process -Force; iex (irm ''https://pastebin.com/raw/HGLwy7XA'')'; Desc = 'Fast first-pass scan for registry activity and logs.'; ButtonType = 'Neutral' },

    @{ Name = 'Red Lotus BAM';         Icon = [char]0xECA5; Kind = 'Command'; Admin = $true;  Cmd = 'Set-ExecutionPolicy Bypass -Scope Process -Force; iex (irm ''https://raw.githubusercontent.com/PureIntent/ScreenShare/main/RedLotusBam.ps1'')'; Desc = 'Inspects Windows execution history using BAM data.'; ButtonType = 'Neutral' },
...
function Hide-FadeElement {
    ...
    $animation.To = 0
    $animation.Duration = [System.Windows.Duration]::new([TimeSpan]::FromMilliseconds($DurationMs))
    $animation.Add_Completed({ $Element.Visibility = "Collapsed" })
    $Element.BeginAnimation([System.Windows.UIElement]::OpacityProperty, $animation)
}

# --- Venster interactie (Slepen en Sluiten) ---
$HeaderBar.Add_MouseLeftButtonDown({ $window.DragMove() })
$CloseButton.Add_Click({ $window.Close() })
$MinButton.Add_Click({ $window.WindowState = 'Minimized' })

# --- Info Paneel logica ---
$InfoButtonTop.Add_Click({ Show-FadeElement $InfoRoot })
$InfoCloseButton.Add_Click({ Hide-FadeElement $InfoRoot })
$InfoOkButton.Add_Click({ Hide-FadeElement $InfoRoot })

# --- Dynamische knoppen genereren voor de Tools ---
foreach ($Tool in $Tools) {
    ...
    if ($Tool.ButtonType -eq 'Primary') {
        $btn.Background = $window.Resources["PrimaryButtonBrush"]
    }

    $btn.Add_Click({
        param($sender, $e)
        $currentTool = $Tools | Where-Object { $_.Name -eq $sender.Content } | Select-Object -First 1
        
        $ActivityBox.AppendText("`n[$(Get-Date -Format 'HH:mm:ss')] Launching: $($currentTool.Name)...")
        $ActivityBox.ScrollToEnd()
        $StatusText.Text = "Running..."
        $StepText.Text = $currentTool.Name
        $MiniStateText.Text = "BUSY"
        $BigChipText.Text = "BUSY"
        $StateChip.Text = "BUSY"
        $FooterText.Text = "Launching"
        $MainProgressBar.Value = 50
        $ProgressLabel.Text = "50%"
        Refresh-Ui

        try {
            if ($currentTool.Kind -eq 'Command') {
                $encodedCommand = [Convert]::ToBase64String([Text.Encoding]::Unicode.GetBytes($currentTool.Cmd))
                if ($currentTool.Admin) {
                    ...
                    Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -EncodedCommand $encodedCommand"
                }
            } elseif ($currentTool.Kind -eq 'Folder') {
                Start-Process -FilePath "explorer.exe" -ArgumentList "`"$($currentTool.Path)`""
            }
            $ActivityBox.AppendText("`n[$(Get-Date -Format 'HH:mm:ss')] Started: $($currentTool.Name)")
        } catch {
            $ActivityBox.AppendText("`n[$(Get-Date -Format 'HH:mm:ss')] Error: $($_.Exception.Message)")
        }

        $ActivityBox.ScrollToEnd()
        $StatusText.Text = "Ready"
        $StepText.Text = "Waiting"
        $MiniStateText.Text = "READY"
        $BigChipText.Text = "READY"
        $StateChip.Text = "READY"
        $FooterText.Text = "Idle"
        $MainProgressBar.Value = 0
        $ProgressLabel.Text = "0%"
    })
    
    [void]$ToolButtonPanel.Children.Add($btn)
}
