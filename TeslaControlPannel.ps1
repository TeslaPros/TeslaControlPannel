...
# ============================================================
# Tesla Control Panel - PREMIUM EDITION (STABLE)
# ============================================================

$AppTitle = 'TESLA CONTROL PANEL'
$Subtitle = 'SYSTEM DIAGNOSTICS & SECURITY'

# Tool definities (Met Segoe MDL2 Icons)
$Tools = @(
...
function Invoke-ToolAction {
    ...
    if ($Tool.Kind -eq 'Folder') {
        try { Start-Process 'explorer.exe' $Tool.Path; Write-Log "Folder geopend: $($Tool.Path)" } catch { }
        return
    }
    try {
...
for ($i = 0; $i -lt $Tools.Count; $i++) {
    ...
    <Border Margin="0,0,0,10" CornerRadius="10" Background="#121418" BorderBrush="#2A2E36" BorderThickness="1">
        <Grid Padding="10">
            ...
            <TextBlock Grid.Column="0" Text="$($t.Icon)" FontFamily="Segoe MDL2 Assets" FontSize="20" Foreground="#00A3FF" VerticalAlignment="Center" HorizontalAlignment="Center"/>
            <StackPanel Grid.Column="1" VerticalAlignment="Center" Margin="10,0">
                <TextBlock Text="$($t.Name)" FontWeight="Bold" Foreground="White" FontSize="14"/>
                <TextBlock Text="$($t.Desc)" Foreground="#666" FontSize="10" TextWrapping="Wrap"/>
            </StackPanel>
            <Button Name="ToolBtn$i" Grid.Column="2" Content="&#xE768;" FontFamily="Segoe MDL2 Assets" FontSize="14" Background="Transparent" Foreground="#00FFA3" BorderThickness="0" Cursor="Hand"/>
        </Grid>
    </Border>
"@
}
...
[xml]$xaml = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" Title="$AppTitle" Width="950" Height="650" WindowStartupLocation="CenterScreen" Background="#0A0A0B" WindowStyle="None" AllowsTransparency="True">
    <Border BorderBrush="#1F2229" BorderThickness="1" CornerRadius="15">
        <Grid>
            ...
            <Border Name="Header" Grid.Row="0" Background="#0D0D0F" CornerRadius="15,15,0,0">
                <Grid Margin="15,0">
                    <TextBlock Text="&#xE950; $AppTitle" FontFamily="Segoe MDL2 Assets" Foreground="#00A3FF" FontSize="16" FontWeight="Bold" VerticalAlignment="Center"/>
                    <StackPanel Orientation="Horizontal" HorizontalAlignment="Right">
                        <Button Name="MinBtn" Content="&#xE921;" FontFamily="Segoe MDL2 Assets" Background="Transparent" Foreground="Gray" Width="35" BorderThickness="0"/>
                        <Button Name="CloseBtn" Content="&#xE8BB;" FontFamily="Segoe MDL2 Assets" Background="Transparent" Foreground="Gray" Width="35" BorderThickness="0"/>
                    </StackPanel>
                </Grid>
            </Border>
            <Grid Grid.Row="1" Margin="15">
                ...
                <Border Grid.Column="0" Background="#0D0D0F" CornerRadius="10" Padding="10" Margin="0,0,10,0">
                    <ScrollViewer VerticalScrollBarVisibility="Hidden"><StackPanel>$toolCards</StackPanel></ScrollViewer>
                </Border>
                <Grid Grid.Column="1">
                    <Grid.RowDefinitions><RowDefinition Height="130"/><RowDefinition Height="*"/></Grid.RowDefinitions>
                    <Border Grid.Row="0" Background="#0D0D0F" CornerRadius="10" Padding="15" Margin="0,0,0,10">
                        <StackPanel>
                            <TextBlock Name="StatusValue" Text="READY" Foreground="#00FFA3" FontSize="28" FontWeight="Black"/>
                            <TextBlock Name="StatusText" Text="Systeem gereed voor gebruik." Foreground="#888" FontSize="12"/>
                        </StackPanel>
                    </Border>
                    <Border Grid.Row="1" Background="#050505" CornerRadius="10" BorderBrush="#1A1A1A" BorderThickness="1">
                        <TextBox Name="LogBox" Background="Transparent" Foreground="#00A3FF" BorderThickness="0" IsReadOnly="True" FontFamily="Consolas" FontSize="11" Margin="10" TextWrapping="Wrap" VerticalScrollBarVisibility="Auto"/>
                    </Border>
                </Grid>
            </Grid>
        </Grid>
...
$Window.FindName('Header').Add_MouseLeftButtonDown({ $Window.DragMove() })
$Window.FindName('CloseBtn').Add_Click({ $Window.Close() })
$Window.FindName('MinBtn').Add_Click({ $Window.WindowState = 'Minimized' })

for ($i = 0; $i -lt $Tools.Count; $i++) {
    $t = $Tools[$i]
    $Window.FindName("ToolBtn$i").Add_Click({ Invoke-ToolAction -Tool $t }.GetNewClosure())
}
